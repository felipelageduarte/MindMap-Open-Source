...
- **1º cadastro = admin.** Rotas: `POST /auth/register|login`, `GET /me`, `GET|POST /maps`, `DELETE|PATCH /maps/{id}`, `GET|PUT /maps/{id}/blob` (URL assinada S3), admin `GET|POST /users`, `DELETE|PATCH /users/{email}`.
